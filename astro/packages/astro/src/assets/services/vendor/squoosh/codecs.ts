...
function resizeNameToIndex(name: 'triangle' | 'catrom' | 'mitchell' | 'lanczos3') {
	switch (name) {
		case 'triangle':
			return 0;
		case 'catrom':
	...
	rotate: {
		...
		instantiate: async () => {
			return async (
				...
				const instance = (await WebAssembly.instantiate(rotateWasm))
...
export const codecs = {
	mozjpeg: {
		...
		dec: () => instantiateEmscriptenWasm(mozDec as DecodeModuleFactory, mozDecWasm),
		enc: () =>
	...
	webp: {
		...
		dec: () => instantiateEmscriptenWasm(webpDec as DecodeModuleFactory, webpDecWasm),
		enc: () =>
			instantiateEmscriptenWasm(
	...
	avif: {
		...
		dec: () => instantiateEmscriptenWasm(avifDec as DecodeModuleFactory, avifDecWasm),
		enc: async () => {
